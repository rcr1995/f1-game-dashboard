...
_TEXT = {
    "en": {
        "tab": "📥 Import race",
        "title": "Import one race",
        "intro": "Upload 2 to 4 PlayStation result screenshots, review every position, then approve one safe workbook update.",
        ...
        "screenshots": "Race result screenshots",
        "screenshots_help": "Upload 2 to 4 screenshots from the same result table. Overlapping rows are reconciled safely.",
        "needs_two": "Upload between 2 and 4 screenshots from this race.",
        ...
        "success": "Race imported safely",
        "ocr_none": "OCR found no standings rows. Remove the screenshots and use a blank review, or try clearer screenshots.",
        "hosted_intro": "Upload 2 to 4 PlayStation result screenshots, review every position, then publish one verified race.",
    ...
    "pt": {
        "tab": "📥 Importar corrida",
        "title": "Importar uma corrida",
        "intro": "Carrega 2 a 4 capturas dos resultados da PlayStation, revê todas as posições e só depois aprova uma atualização segura do Excel.",
        ...
        "screenshots_help": "Carrega 2 a 4 capturas da mesma tabela. As linhas repetidas entre imagens serão reconciliadas.",
        "needs_two": "Carrega entre 2 e 4 capturas desta corrida.",
        ...
        "success": "Corrida importada com segurança",
        "ocr_none": "O OCR não encontrou linhas. Remove as capturas e usa a revisão vazia, ou tenta imagens mais nítidas.",
        "hosted_intro": "Carrega 2 a 4 capturas dos resultados da PlayStation, revê todas as posições e publica uma corrida verificada.",
...
def valid_screenshot_count(count: int) -> bool:
    """Return whether one race submission has an allowed screenshot count."""
    return 2 <= count <= 4
...
def validate_screenshot_set(upload_bytes: list[bytes]) -> list[str]:
    ...
    if not valid_screenshot_count(len(upload_bytes)):
        errors.append("A race upload requires between 2 and 4 screenshots.")
...
def _event_defaults(
    ...
    upcoming = (
        league_calendar[league_calendar["Status"].astype(str).str.casefold().eq("upcoming")]
        if not league_calendar.empty
        else league_calendar
    )
    if not upcoming.empty:
        ...
        matching = (
            league_calendar[pd.to_numeric(league_calendar["Round"], errors="coerce").eq(default_round)]
            if not league_calendar.empty
            else league_calendar
        )
        default_gp = str(matching.iloc[0]["GP Name"]) if not matching.empty else ""
    gp_options = (
        [str(value) for value in league_calendar["GP Name"].dropna().unique().tolist()]
        if not league_calendar.empty
        else []
    )
    historical_gps = [
        str(value)
        for value in data["GP Name"].dropna().unique().tolist()
        if str(value) != "Season Final"
    ]
    return default_round, default_gp, list(
        dict.fromkeys(([default_gp] if default_gp else []) + gp_options + sorted(historical_gps))
    )
...
def render_race_import(
    ...
    *,
    ...
    championship = st.selectbox(
        ...
        options,
        index=_default_championship(options, standings),
        format_func=_format_championship,
        key="race_import_championship",
    )
    ...
    default_round, default_gp, gp_options = _event_defaults(
        standings,
        calendar,
        game,
        season,
    ...
    with metadata_columns[0]:
        round_number = int(
            st.number_input(
                ...
                value=default_round,
                step=1,
                key=f"race_import_round_{context_key}",
            )
        )
    matching_calendar = (
        calendar[
    ...
    with metadata_columns[1]:
        gp_name = (
            st.selectbox(
                ...
                key=f"race_import_gp_{context_key}_{round_number}",
            )
            if gp_options
            else st.text_input(
                text(lang, "custom_gp"),
                value=round_gp,
                key=f"race_import_gp_text_{context_key}_{round_number}",
            )
        )
    with metadata_columns[2]:
        display_type = st.radio(
            text(lang, "event_type"),
            [text(lang, "race"), text(lang, "sprint")],
            horizontal=True,
            key=f"race_import_type_{context_key}_{round_number}",
        )
    ...
    uploads = st.file_uploader(
        ...
        key=(
            f"race_import_uploads_{context_key}_{round_number}_{event_type}_"
    ...
    if extract_clicked:
        ...
        try:
            ...
            st.session_state["race_import_draft"] = {
                ...
                "draft_id": ri.review_digest(review_rows, context)[:12],
            }
