...
T = {
    "en": {
        ...
        "using_bundled": "Using bundled file if present (F1_Standings.xlsx).",
        ...
        "share_body": "Best private option: run this app on your PC and use Tailscale so your brothers can open it from home.",
    },
    "pt": {
        "title": "Dashboard F1",
        "data": "Dados",
        ...
        "using_bundled": "A usar o ficheiro local se existir (F1_Standings.xlsx).",
        ...
        "share_body": "Melhor opção privada: corre a app no teu PC e usa Tailscale para os teus irmãos abrirem a app a partir de casa.",
    },
}
...
with st.sidebar:
    ...
    if uploaded is None:
        st.caption(tr(lang, "using_bundled"))
        default_path = "F1_Standings.xlsx"
        try:
            raw = load_data_from_excel(default_path)
        except Exception:
            st.error("Upload your Excel file (couldn't find F1_Standings.xlsx next to the app).")
            st.stop()
    else:
        ...
        with right:
            ...
            if rr.empty:
                ...
                leaders = leaders.merge(
                    second[["EventIdx","CumPoints"]].rename(columns={"CumPoints":"SecondPoints"}),
                    on="EventIdx", how="left"
                )
...
with tab_sim:
    if df.empty:
        ...
        with sim2:
            ...
            comp = base_st[[entity_col,"Points","Pos"]].merge(
                scen_st[[entity_col,"Points","Pos"]].rename(columns={"Points":"ScenarioPoints","Pos":"ScenarioPos"}),
                on=entity_col, how="outer"
            ).fillna({"Points":0,"ScenarioPoints":0})
...
with tab_settings:
    ...
    st.markdown(
        """
**Private sharing (recommended): Tailscale**
1) Install Tailscale on your PC and on your brothers' PCs.
2) Log in with the same account (or invite them to your tailnet).
3) On your PC, run:
   - `python -m streamlit run f1_standings_app_v6.py --server.address 0.0.0.0 --server.port 8501`
4) In Tailscale, copy your PC's **Tailscale IP** (looks like 100.x.y.z).
5) Your brothers open:
   - `http://YOUR_TAILSCALE_IP:8501`

Tip: your PC must be on while they use it.
"""
    )

    st.markdown(f"### {tr(lang, 'safe_delete')}")
    st.write([
        "f1_standings_app.py","requirements.txt",
        "f1_standings_app_v2.py","requirements_v2.txt",
        "f1_standings_app_v3.py","requirements_v3.txt",
