...
T = {
    "en": {
        ...
        "season": "Season",
        "league": "League",
        "tabs": ["🏁 Dashboard", "🗓️ Calendar", "📊 GP Statistics", "🛣️ Circuits", "🏆 All‑time", "⚙️ Settings"],
        "standings_type": "Standings type",
        "drivers": "Drivers",
        "constructors": "Constructors",
        "overview": "Overview",
    ...
    "pt": {
        ...
        "season": "Época",
        "league": "Liga",
        "tabs": ["🏁 Dashboard", "🗓️ Calendário", "📊 Estatísticas GP", "🛣️ Circuitos", "🏆 Histórico", "⚙️ Definições"],
        "standings_type": "Tipo de classificação",
        "drivers": "Pilotos",
        "constructors": "Construtores",
        "overview": "Resumo",
...
tab_dash, tab_calendar, tab_gp, tab_circuits, tab_all, tab_settings = st.tabs(tr(lang, "tabs"))
...
with tab_calendar:
    st.caption(f"{tr(lang, 'current_season_only')}: {latest_meta['SeasonLabel']} • {latest_meta['League Name']}")
    ...
    if cal.empty:
        ...
        if cal.empty:
            ...
            st.dataframe(cal[show_cols], use_container_width=True, hide_index=True)
    ...
    if df_gp.empty:
        ...
        if not wide.empty and not long.empty:
            ...
            if PLOTLY_OK:
                ...
                fig.update_layout(height=500, margin=dict(l=10, r=10, t=10, b=10), legend_title_text="")
                fig.update_xaxes(type="category", title=("Timeline" if gp_all_time else "GPs"))
                st.plotly_chart(fig, use_container_width=True)
            else:
                st.line_chart(wide[keep].ffill().fillna(0), height=500)
        ...
        if not gp_tension_df.empty and PLOTLY_OK:
            ...
            tdf["EventLabel"] = "R" + tdf["EventIdx"].astype(int).astype(str)
            fig_tension = go.Figure()
            fig_tension.add_trace(go.Scatter(x=tdf["EventLabel"], y=tdf["GapP1P2"], mode="lines+markers", name=tr(lang, "gap_p1_p2"), line={"color": THEME_CFG["positive"]}))
            fig_tension.add_trace(go.Scatter(x=tdf["EventLabel"], y=tdf["Top3Spread"], mode="lines+markers", name=tr(lang, "gap_top3"), line={"color": THEME_CFG["neutral"]}))
            fig_tension.update_layout(template=THEME_CFG["plotly_template"], height=360, margin=dict(l=10, r=10, t=10, b=10))
            st.plotly_chart(fig_tension, use_container_width=True)
        elif not gp_tension_df.empty:
            st.line_chart(gp_tension_df.set_index("EventIdx")[["GapP1P2", "Top3Spread"]], height=360)
        else:
...
with tab_all:
    if base_all.empty:
        ...
        base = base_all.copy()
        base["SeasonYear"] = (
            base["SeasonLabel"].astype(str).str.extract(r"(\d{4})", expand=False)
            .fillna(base["SeasonLabel"].astype(str))
        )
        ...
        if PLOTLY_OK:
            fig = px.line(
                ...
                color=entity,
                markers=True,
                category_orders={"SeasonYear": season_order, entity: keep},
                color_discrete_sequence=THEME_CFG["line_palette"],
                template=THEME_CFG["plotly_template"],
            )
        ...
        st.subheader(tr(lang, "all_time_standings"))
        table_df = totals[["SeasonYear", entity, "Points", "CumPoints"]].sort_values(
            ["SeasonYear", "Points"],
