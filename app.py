...
def event_sort_cols(df: pd.DataFrame, all_time: bool) -> pd.DataFrame:
    d = df.copy()
    if all_time:
        d["EventIdx"] = d["SeasonNum"].fillna(0).astype(int) * 1000 + d["Round"].astype(int)
        d["EventLabel"] = d["Season"].astype(str) + " R" + d["Round"].astype(str) + " • " + np.where(d.get("IsSeasonFinal", False), season_final_label(), d["GP Name"])
    else:
        d["EventIdx"] = d["Round"].astype(int)
        d["EventLabel"] = "R" + d["Round"].astype(str) + " • " + np.where(d.get("IsSeasonFinal", False), season_final_label(), d["GP Name"])
    return d
...
# -----------------------------
# Filters (single-select, linked) — default All
# -----------------------------

pairs = (
    raw[["Game", "SeasonLabel", "League Name", "SeasonNum"]]
    .drop_duplicates()
    .sort_values(["Game", "SeasonNum", "SeasonLabel", "League Name"], na_position="last")
)

games_all = sorted(pairs["Game"].dropna().unique().tolist())
game_options = ["All"] + games_all

# Session-state keys to make linking stable
if "flt_game" not in st.session_state:
    st.session_state["flt_game"] = "All"
if "flt_season" not in st.session_state:
    st.session_state["flt_season"] = "All"
if "flt_league" not in st.session_state:
    st.session_state["flt_league"] = "All"

c1, c2, c3 = st.columns([1, 1, 1], gap="large")

with c1:
    st.selectbox(tr(lang, "game"), game_options, key="flt_game")

# Filter available pairs by game (unless All)
pairs_g = pairs if st.session_state["flt_game"] == "All" else pairs[pairs["Game"] == st.session_state["flt_game"]]

# Build Season / League options
season_options = ["All"] + sorted(pairs_g["SeasonLabel"].dropna().unique().tolist())
league_options = ["All"] + sorted(pairs_g["League Name"].dropna().unique().tolist())

with c2:
    st.selectbox(tr(lang, "season"), season_options, key="flt_season")

# If Season selected, force League to the linked one (1-to-1)
if st.session_state["flt_season"] != "All":
    linked = pairs_g[pairs_g["SeasonLabel"] == st.session_state["flt_season"]]["League Name"].dropna().unique().tolist()
    if linked:
        linked_league = linked[0]
        if st.session_state["flt_league"] not in ("All", linked_league):
            st.session_state["flt_league"] = linked_league
            st.rerun()

with c3:
    # If season selected, league options collapse to linked league + All
    if st.session_state["flt_season"] != "All":
        linked = pairs_g[pairs_g["SeasonLabel"] == st.session_state["flt_season"]]["League Name"].dropna().unique().tolist()
        league_options_eff = ["All"] + linked if linked else league_options
    else:
        league_options_eff = league_options
    st.selectbox(tr(lang, "league"), league_options_eff, key="flt_league")

# If League selected, force Season to linked (1-to-1)
if st.session_state["flt_league"] != "All":
    linked = pairs_g[pairs_g["League Name"] == st.session_state["flt_league"]]["SeasonLabel"].dropna().unique().tolist()
    if linked:
        linked_season = linked[0]
        if st.session_state["flt_season"] not in ("All", linked_season):
            st.session_state["flt_season"] = linked_season
            st.rerun()

# Apply filters to data
df_filtered = raw.copy()

if st.session_state["flt_game"] != "All":
    df_filtered = df_filtered[df_filtered["Game"] == st.session_state["flt_game"]]

if st.session_state["flt_season"] != "All":
    df_filtered = df_filtered[df_filtered["SeasonLabel"] == st.session_state["flt_season"]]

if st.session_state["flt_league"] != "All":
    df_filtered = df_filtered[df_filtered["League Name"] == st.session_state["flt_league"]]

df_filtered = raw.copy()
if selected_games:
    df_filtered = df_filtered[df_filtered["Game"].isin(selected_games)]
if selected_pairs:
    season_labels = [p.split(" — ", 1)[0] for p in selected_pairs]
    df_filtered = df_filtered[df_filtered["SeasonLabel"].isin(season_labels)]
...
with tab_dash:
    if df.empty:
        ...
        st.caption(f"{('All' if len(selected_games)==len(games) else ', '.join(selected_games))} • {('All' if len(selected_pairs)==len(sorted(pairs[pairs['Game'].isin(selected_games)]['SeasonLeague'].unique().tolist())) else ', '.join(selected_pairs))}")
        ...
        with left:
            ...
            if last_tbl.empty:
                st.caption(tr(lang, "not_enough") + " (" + season_final_label() + " excluded)")
            else:
        ...
        with right:
            ...
            metric = st.radio("Progress metric", ["Cumulative points", "Rank (position)"], horizontal=True, key="prog_metric")

            # If multiple seasons/leagues are in view, build a cross-season timeline
            use_all_time = df["SeasonLabel"].nunique() > 1 or st.session_state.get("flt_season","All") == "All" or st.session_state.get("flt_league","All") == "All"

            wide, long = cumulative_points_wide(df, entity_col=entity_col, all_time=use_all_time)
            if wide.empty or long.empty:
                st.info(tr(lang, "not_enough"))
            else:
                # Keep top N entities by last available cumulative points
                last_idx = wide.index.max()
                final = wide.tail(1).T.sort_values(by=last_idx, ascending=False)
                keep = final.head(top_n).index.tolist()

                if metric == "Cumulative points":
                    long_k = long[long[entity_col].isin(keep)].copy()

                    # Order x as categorical so we show every event label (not just endpoints)
                    order = (
                    ...
                    if PLOTLY_OK:
                        fig = px.line(long_k, x="EventLabel", y="CumPoints", color=entity_col, markers=True,
                                      category_orders={"EventLabel": order})
                    ...
                    # Rank progression: compute position each event by cumulative points
                    rr = event_sort_cols(df, all_time=use_all_time).sort_values(["EventIdx","GP Name", entity_col])
                    rr["CumPoints"] = rr.groupby(entity_col)["Points"].cumsum()
                    per = rr.groupby([entity_col,"EventIdx","EventLabel"], as_index=False)["CumPoints"].max()
                    per["Position"] = per.groupby("EventIdx")["CumPoints"].rank(method="min", ascending=False).astype(int)

                    per = per[per[entity_col].isin(keep)].copy()

                    order = (
                        per.sort_values("EventIdx")[["EventIdx","EventLabel"]]
                        .drop_duplicates()
                        .sort_values("EventIdx")["EventLabel"]
                        .tolist()
                    )
                    ...
                    if PLOTLY_OK:
                        fig = px.line(per, x="EventLabel", y="Position", color=entity_col, markers=True,
                                      category_orders={"EventLabel": order})
                        fig.update_layout(height=360, margin=dict(l=10,r=10,t=10,b=10), legend_title_text="")
                        fig.update_xaxes(type="category")
                        fig.update_yaxes(autorange="reversed")  # 1 at top
                        st.plotly_chart(fig, use_container_width=True)
                    else:
                        # Fallback: show final table
                        st.dataframe(per.sort_values(["EventIdx","Position"]), use_container_width=True, hide_index=True)
...
with tab_all:
    if raw.empty:
        ...
        with c2:
            ...
            st.markdown("### Season-to-season history")
            base_eff = effective_rows(base)
            # Build season totals per entity (works for both per-GP seasons and Season Final seasons)
            season_totals = (
                base_eff.groupby(["SeasonLabel", "SeasonNum", entity_col], as_index=False)
                        .agg(Points=("Points","sum"),
                             FinishPos=("Finish Pos","min"))
            )
            # Sort seasons: numeric year if available, then label
            season_totals["SeasonSort"] = season_totals["SeasonNum"].fillna(9999)
            season_totals = season_totals.sort_values(["SeasonSort","SeasonLabel"])

            options = sorted(season_totals[entity_col].dropna().unique().tolist())
            picks = st.multiselect("Compare up to 3", options, default=options[:1], max_selections=3)

            if picks:
                plot_df = season_totals[season_totals[entity_col].isin(picks)].copy()
                x_col = "SeasonLabel"
                if PLOTLY_OK and not plot_df.empty:
                    season_order = sorted(season_totals["SeasonLabel"].dropna().unique().tolist())
                    fig3 = px.line(
                        plot_df,
                        x=x_col,
                        y="Points",
                        color=entity_col,
                        markers=True,
                        hover_data={"FinishPos": True, "SeasonLabel": True, "Points": True},
                        category_orders={"SeasonLabel": season_order},
                    )
                    fig3.update_layout(height=320, margin=dict(l=10,r=10,t=10,b=10), legend_title_text="")
                    # Force all seasons to appear as categorical ticks (not just first/last)
                    fig3.update_xaxes(
                        type="category",
                        categoryorder="array",
                        categoryarray=season_order,
                        tickmode="array",
                        tickvals=season_order,
                    )
                    st.plotly_chart(fig3, use_container_width=True)
                else:
                    # fallback: pivot
                    piv = plot_df.pivot_table(index="SeasonLabel", columns=entity_col, values="Points", aggfunc="sum").fillna(0)
                    st.line_chart(piv, height=320)

                with st.expander("Season totals table"):
                    st.dataframe(plot_df.sort_values(["SeasonSort","SeasonLabel","Points"], ascending=[True,True,False]),
                                 use_container_width=True, hide_index=True)
                    download_csv_button(plot_df, "season_history.csv", tr(lang, "download"))
