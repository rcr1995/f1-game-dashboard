...
def load_data_from_excel(file) -> pd.DataFrame:
    ...
    - Per-GP rows (normal): GP Name not in season-total markers
    - Season-total rows: GP Name in {'All','Season Total','Final','Season Final'}
    - Season can be numeric (2024) or text ('2014/15', '2014-2015')

    Adds:
      SeasonLabel (string) for filtering/display
      SeasonNum (numeric) for ordering/sorting when available
    """
    ...
    # Clean strings
    for c in ["Game","Season","League Name","GP Name","Driver","Team"]:
    ...
    # If SeasonLabel is a clean integer year, prefer it
    ...
    # Keep rows with Round present (needed for ordering)
...
if season != "All":
    df_filtered = df_filtered[df_filtered["Season"].astype(int) == int(season)]
if league != "All":
