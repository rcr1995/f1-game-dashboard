...
def last_gp_table(df: pd.DataFrame, entity: str):
    ...
    if entity == "Drivers":
        t = d.groupby(["Driver","Team"], as_index=False).agg(
            Points=("Points","sum"),
            Pos=("Finish Pos","min"),
        ).sort_values(["Pos","Driver"], ascending=[True,True])
        t.insert(0, "Pos", range(1, len(t)+1))
        # Drop duplicated position column if present
        if "FinishPos" in t.columns:
            t = t.drop(columns=["FinishPos"])
...
with right:
            ...
            metric = st.radio(
                "Progress metric",
                ["Cumulative points", "Rank (position)"],
                horizontal=True,
                key="prog_metric",
            )

            # Granularity applies to BOTH metrics (so rank can be shown season-by-season etc.)
            gran = st.radio(
                "Progress by",
                ["League", "Season", "Game"],
                horizontal=True,
                key="prog_granularity",
            )

            # Build X-axis buckets and ordering
            if gran == "League":
                # In your data, League is 1-to-1 with SeasonLabel (e.g., 2019-T01, 2019-T02 ...)
                ...
                x_title = "League timeline (SeasonLabel)"

            elif gran == "Season":
                tmp = df.copy()
                tmp["SeasonYear"] = tmp["SeasonLabel"].astype(str).str.extract(r"(\d{4})", expand=False)
                tmp["SeasonYear"] = tmp["SeasonYear"].fillna(tmp["SeasonLabel"].astype(str))
            ...
            if base.empty:
                ...
                # Keep top N by total points over the selected bucketed timeline
                ...
                base = base.sort_values(["X", entity_col])

                # Cumulative across X
                base["CumPoints"] = base.groupby(entity_col)["Points"].cumsum()

                if metric == "Cumulative points":
                    # Legend order by final cumulative points (leader on top)
                    ...
                    if PLOTLY_OK:
                        ...
                        wide_plot = (
                            base.pivot_table(index="X", columns=entity_col, values="CumPoints", aggfunc="max")
                            .fillna(0)
                        )
                        st.line_chart(wide_plot, height=360)

                else:
                    # Rank (position) over the same cumulative buckets
                    per = base.copy()
                    per["Position"] = per.groupby("X")["CumPoints"].rank(method="min", ascending=False).astype(int)

                    order_x = x_order

                    last_pos = (
                        per.sort_values("X")
                        .groupby(entity_col)["Position"]
                        .last()
                    ...
                    if PLOTLY_OK:
                        fig = px.line(
                            ...
                            color=entity_col,
                            markers=True,
                            category_orders={"X": order_x, entity_col: order_entities},
                        )
                        fig.update_layout(
                            height=360,
                            margin=dict(l=10, r=10, t=10, b=10),
                        ...
                        st.dataframe(per.sort_values(["X", "Position"]), use_container_width=True, hide_index=True)
with tab_matrix:
    if df.empty:
        st.info(tr(lang, "no_rows"))
    else:
