...
with c1:
    all_games = st.checkbox("All games", value=False, key="all_games_chk")
    default_sel_games = games if all_games else default_games
    selected_games = st.multiselect(tr(lang, "game"), games, default=default_sel_games, key="games_ms")
    # If user clears selection, treat it as All
    if not selected_games:
        selected_games = games

with c2:
    avail_pairs = pairs[pairs["Game"].isin(selected_games)] if selected_games else pairs
    label_pairs = f"{tr(lang,'season')} — {tr(lang,'league')}"
    all_pairs = st.checkbox("All seasons/leagues", value=False, key="all_pairs_chk")
    all_pair_list = sorted(avail_pairs["SeasonLeague"].unique().tolist())
    default_sel_pairs = all_pair_list if all_pairs else [p for p in default_pairs if p in all_pair_list]
    if not default_sel_pairs and all_pair_list:
        # fallback: latest within current game selection
        default_sel_pairs = all_pair_list[-1:]
    selected_pairs = st.multiselect(label_pairs, all_pair_list, default=default_sel_pairs, key="pairs_ms")
    if not selected_pairs:
        selected_pairs = all_pair_list
