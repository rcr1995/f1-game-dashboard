...
with tab_dash:
    ...
    sel_gp_pair = st.selectbox("Season — League (GP-only)", seasonleague_options, index=0, key="gp_pair")
    gp_season, gp_league = sel_gp_pair.split(" — ", 1)

    # Filter raw to selected Season/League and to GP rows only
    df_gp = raw.copy()
    if "IsSeasonFinal" not in df_gp.columns:
        df_gp = mark_season_final(df_gp)
    df_gp = df_gp[(df_gp["SeasonLabel"] == gp_season) & (df_gp["League Name"] == gp_league) & (~df_gp["IsSeasonFinal"])].copy()
    ...
    if df.empty:
        ...
        with right:
            ...
            wide, long = cumulative_points_wide(df_stats, entity_col=entity_col, all_time=False)
            if wide.empty or long.empty:
                ...
                if PLOTLY_OK:
                    ...
                    fig.update_xaxes(type="category", title="GPs")
...
with tab_all:
    if raw.empty:
        ...
        if base_gp.empty:
            ...
            season_driver = (
                base_gp.groupby(["SeasonLabel", "SeasonNum", "Driver"], as_index=False)
                      .agg(Points=("Points","sum"),
                           Races=("GP Name","nunique"),
                           Wins=("Finish Pos", lambda s: int((s==1).sum())))
            )
            season_driver["_SortKey"] = season_driver["SeasonNum"].fillna(season_driver["SeasonLabel"].map(_season_sort_key))
            season_driver = season_driver.sort_values(["_SortKey", "Points"], ascending=[False, False])

            st.caption("Ranks drivers by total points per season (one row per driver per season).")
            st.dataframe(season_driver.drop(columns=["_SortKey"], errors="ignore"), use_container_width=True, hide_index=True)
            download_csv_button(season_driver.drop(columns=["_SortKey"], errors="ignore"), "all_time_season_driver_totals.csv", tr(lang, "download"))
with tab_settings:
