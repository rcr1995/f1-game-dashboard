...
with c1:
    selected_games = st.multiselect(tr(lang, "game"), games, default=default_games)
with c2:
    avail_pairs = pairs[pairs["Game"].isin(selected_games)] if selected_games else pairs
    label_pairs = f"{tr(lang,'season')} — {tr(lang,'league')}"
    selected_pairs = st.multiselect(label_pairs, sorted(avail_pairs["SeasonLeague"].unique().tolist()), default=default_pairs)
