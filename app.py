...
with tab_dash:
    ...
    if df.empty:
        ...
        c3.metric(tr(lang, "races"), f"{int(df_stats['GP Name'].nunique())}")
...
with tab_all:
    if raw.empty:
        ...
        st.subheader("All‑time: Season totals leaderboard")

        base = raw.copy()
        if "IsSeasonFinal" not in base.columns:
            base = mark_season_final(base)
        base_gp = base[~base["IsSeasonFinal"]].copy()
        if base_gp.empty:
            st.info(tr(lang, "not_enough"))
        else:
            base_gp = apply_points_override(base_gp, mode=canonical_points_mode, custom_map=custom_map, last_round_multiplier=1.0)

            # Collapse SeasonLabel like '2019-T01' into SeasonYear '2019'
            base_gp["SeasonYear"] = base_gp["SeasonLabel"].astype(str).str.extract(r"(\d{4})", expand=False).fillna(base_gp["SeasonLabel"].astype(str))

            season_driver = (
                base_gp.groupby(["SeasonYear", "Driver"], as_index=False)
                       .agg(Points=("Points","sum"),
                            Races=("GP Name","nunique"),
                            Wins=("Finish Pos", lambda s: int((s==1).sum())))
            )

            # Order seasons chronologically when possible
            def _year_key(v):
                mm = re.search(r"(\d{4})", str(v))
                return int(mm.group(1)) if mm else 0
            season_order = sorted(season_driver["SeasonYear"].unique().tolist(), key=_year_key)

            season_driver = season_driver.sort_values(["SeasonYear","Points"], ascending=[True, False])
            st.caption("Ranks drivers by total points per season (SeasonYear aggregates all leagues within that season).")
            st.dataframe(season_driver, use_container_width=True, hide_index=True)
            download_csv_button(season_driver, "all_time_season_driver_totals.csv", tr(lang, "download"))

            st.divider()
            st.subheader("Season progression (cumulative points)")
            top_n = st.slider("Top N drivers", 5, 30, 10, key="alltime_topn")
            totals = season_driver.groupby("Driver")["Points"].sum().sort_values(ascending=False)
            keep = totals.head(top_n).index.tolist()
            plot_df = season_driver[season_driver["Driver"].isin(keep)].copy()
            plot_df["SeasonYear"] = pd.Categorical(plot_df["SeasonYear"], categories=season_order, ordered=True)
            plot_df = plot_df.sort_values(["SeasonYear"])

            # Legend order by total points
            order_entities = totals.head(top_n).index.tolist()

            if PLOTLY_OK:
                fig = px.line(plot_df, x="SeasonYear", y="Points", color="Driver", markers=True,
                              category_orders={"SeasonYear": season_order, "Driver": order_entities})
                fig.update_layout(height=360, margin=dict(l=10, r=10, t=10, b=10), legend_title_text="")
                fig.update_xaxes(type="category", title="Season")
                st.plotly_chart(fig, use_container_width=True)
            else:
                wide_plot = plot_df.pivot_table(index="SeasonYear", columns="Driver", values="Points", aggfunc="sum").fillna(0)
                st.line_chart(wide_plot, height=360)
