...
with tab_dash:
    ...
    if df.empty:
        ...
        with right:
            ...
            timeline = st.selectbox("Timeline", ["League (event-by-event)", "Season totals", "Game totals"], index=0, key="prog_timeline")

            if timeline == "League (event-by-event)":
                ...
                if wide.empty or long.empty:
                    ...
                    if metric == "Cumulative points":
                        ...
                        # order x labels and legend by latest cum points
                        order_x = (
            ...
            elif timeline == "Season totals":
                sp = season_totals(df, entity_col=entity_col)
                if sp.empty:
                    st.info(tr(lang, "not_enough"))
                else:
                    order_x = sorted(sp["SeasonLabel"].unique().tolist(), key=_season_sort_key)
                    # legend order by latest season points
                    last_pts = sp.sort_values("SeasonLabel", key=lambda s: s.map(_season_sort_key)).groupby(entity_col)["Points"].last().sort_values(ascending=False)
                    order_entities = last_pts.index.tolist()
                    sp_k = sp.copy()

                    if PLOTLY_OK:
                        fig = px.line(sp_k, x="SeasonLabel", y="Points", color=entity_col, markers=True,
                                      category_orders={"SeasonLabel": order_x, entity_col: order_entities})
                        fig.update_layout(height=360, margin=dict(l=10,r=10,t=10,b=10), legend_title_text="")
                        fig.update_xaxes(type="category")
                        st.plotly_chart(fig, use_container_width=True)
                    else:
                        wide_plot = sp_k.pivot_table(index="SeasonLabel", columns=entity_col, values="Points", aggfunc="sum").fillna(0)
                        st.line_chart(wide_plot, height=360)

            else:
                gp = game_totals(df, entity_col=entity_col)
                if gp.empty:
                    st.info(tr(lang, "not_enough"))
                else:
                    order_x = sorted(gp["Game"].unique().tolist())
                    last_pts = gp.groupby(entity_col)["Points"].sum().sort_values(ascending=False)
                    order_entities = last_pts.index.tolist()
                    if PLOTLY_OK:
                        fig = px.line(gp, x="Game", y="Points", color=entity_col, markers=True,
                                      category_orders={"Game": order_x, entity_col: order_entities})
                        fig.update_layout(height=360, margin=dict(l=10,r=10,t=10,b=10), legend_title_text="")
                        fig.update_xaxes(type="category")
                        st.plotly_chart(fig, use_container_width=True)
                    else:
                        wide_plot = gp.pivot_table(index="Game", columns=entity_col, values="Points", aggfunc="sum").fillna(0)
                        st.line_chart(wide_plot, height=360)
