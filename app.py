                ...
                if metric == "Cumulative points":
                    ...
                    if PLOTLY_OK:
                        fig = px.line(
                            ...
                            category_orders={"EventLabel": order, entity_col: keep}
                        )
                    ...
                    # Order legend by current rank (1 on top)
                    try:
                        _last_ev = per["EventIdx"].max()
                        keep_rank = per[per["EventIdx"] == _last_ev].sort_values("Position")[entity_col].tolist()
                    except Exception:
                        keep_rank = keep
                    ...
                    if PLOTLY_OK:
                        ...
                        category_orders={"EventLabel": order, entity_col: keep_rank}
                    )
                        fig.update_layout(height=360, margin=dict(l=10,r=10,t=10,b=10), legend_title_text="")
                        fig.update_xaxes(type="category")
