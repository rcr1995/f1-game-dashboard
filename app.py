...
with tab_dash:
    ...
    if df.empty:
        ...
        with right:
            ...
            timeline = st.selectbox("Timeline", ["League", "Season", "Game"], index=0, key="prog_timeline")

            if metric == "Rank (position)":
                # Rank progression only makes sense event-by-event (within a league/season)
                wide, long = cumulative_points_wide(df, entity_col=entity_col, all_time=True)
                if wide.empty or long.empty:
                    ...
                    rr = event_sort_cols(df, all_time=True).sort_values(["EventIdx", "GP Name", entity_col])
                    ...
                    last_pos = per.sort_values("EventIdx").groupby(entity_col)["Position"].last().sort_values(ascending=True)
                    order_entities = last_pos.index.tolist()
                ...
                # Cumulative points by League / Season / Game (stepwise, across selections)
                if timeline == "League":
                    # League is 1-to-1 with SeasonLabel in your data, so SeasonLabel is the league timeline anchor.
                    base = df.groupby(["SeasonLabel", entity_col], as_index=False)["Points"].sum()
                    base = base.rename(columns={"SeasonLabel": "X"})
                    x_order = sorted(base["X"].unique().tolist(), key=_season_sort_key)

                elif timeline == "Season":
                    # Collapse SeasonLabel like '2019-T01' into '2019' (season year). If no year, keep label.
                    tmp = df.copy()
                    tmp["SeasonYear"] = tmp["SeasonLabel"].astype(str).str.extract(r"(\d{4})", expand=False).fillna(tmp["SeasonLabel"].astype(str))
                    base = tmp.groupby(["SeasonYear", entity_col], as_index=False)["Points"].sum()
                    base = base.rename(columns={"SeasonYear": "X"})
                    # order seasons numerically when possible
                    def _year_key(v):
                        m = re.search(r"(\d{4})", str(v))
                        return int(m.group(1)) if m else 0
                    x_order = sorted(base["X"].unique().tolist(), key=_year_key)

                else:  # Game
                    base = df.groupby(["Game", entity_col], as_index=False)["Points"].sum()
                    base = base.rename(columns={"Game": "X"})
                    x_order = sorted(base["X"].unique().tolist())

                if base.empty:
                    st.info(tr(lang, "not_enough"))
                else:
                    # Keep top N by total points across selected X
                    totals = base.groupby(entity_col)["Points"].sum().sort_values(ascending=False)
                    keep = totals.head(top_n).index.tolist()
                    base = base[base[entity_col].isin(keep)].copy()

                    # Build cumulative across X (ordered)
                    base["X"] = pd.Categorical(base["X"], categories=x_order, ordered=True)
                    base = base.sort_values(["X"])

                    base["CumPoints"] = base.groupby(entity_col)["Points"].cumsum()

                    # Legend order by final cumulative points
                    last_pts = base.groupby(entity_col)["CumPoints"].max().sort_values(ascending=False)
                    order_entities = last_pts.index.tolist()

                    if PLOTLY_OK:
                        fig = px.line(
                            base, x="X", y="CumPoints", color=entity_col, markers=True,
                            category_orders={"X": x_order, entity_col: order_entities}
                        )
                        fig.update_layout(height=360, margin=dict(l=10, r=10, t=10, b=10), legend_title_text="")
                        fig.update_xaxes(type="category", title=timeline)
                        st.plotly_chart(fig, use_container_width=True)
                    else:
                        wide_plot = base.pivot_table(index="X", columns=entity_col, values="CumPoints", aggfunc="max").fillna(0)
