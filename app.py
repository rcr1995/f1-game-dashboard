...
# --- Single-select filters (default All), Season ↔ League linked (1-to-1) ---
game_options = ["All"] + games

# pick game
f1, f2, f3 = st.columns([1, 1, 1], gap="large")
with f1:
    game_sel = st.selectbox(tr(lang, "game"), game_options, index=0, key="flt_game_single")

pairs_g = pairs if game_sel == "All" else pairs[pairs["Game"] == game_sel]
...
with f2:
    season_sel = st.selectbox(tr(lang, "season"), season_options, index=0, key="flt_season_single")

# If Season selected, force League to the linked one
if season_sel != "All":
    linked_league = pairs_g.loc[pairs_g["SeasonLabel"] == season_sel, "League Name"].dropna().unique().tolist()
    league_options_eff = ["All"] + linked_league if linked_league else league_options
else:
    league_options_eff = league_options

with f3:
    league_sel = st.selectbox(tr(lang, "league"), league_options_eff, index=0, key="flt_league_single")

# If user picks League (and Season is All), auto-set Season; or if Season picked, auto-set League
# (rerun to update UI state safely)
if season_sel == "All" and league_sel != "All":
    linked_season = pairs_g.loc[pairs_g["League Name"] == league_sel, "SeasonLabel"].dropna().unique().tolist()
    if linked_season:
        st.session_state["flt_season_single"] = linked_season[0]
        st.rerun()

if season_sel != "All" and league_sel == "All":
    linked_league = pairs_g.loc[pairs_g["SeasonLabel"] == season_sel, "League Name"].dropna().unique().tolist()
    if linked_league:
        st.session_state["flt_league_single"] = linked_league[0]
        st.rerun()
...
with tab_dash:
    if df.empty:
        ...
        with right:
            ...
            use_all_time = (
                st.session_state.get("flt_season_single", "All") == "All"
                or st.session_state.get("flt_league_single", "All") == "All"
                or df["SeasonLabel"].nunique() > 1
            )
            ...
            if wide.empty or long.empty:
                ...
                if metric == "Cumulative points":
                    ...
                    if PLOTLY_OK:
                        fig = px.line(
                            long_k, x="EventLabel", y="CumPoints", color=entity_col, markers=True,
                    ...
                    if PLOTLY_OK:
                        fig = px.line(
                            per, x="EventLabel", y="Position", color=entity_col, markers=True,
                            category_orders={"EventLabel": order}
                        )
                        fig.update_layout(height=360, margin=dict(l=10,r=10,t=10,b=10), legend_title_text="")
                        fig.update_xaxes(type="category")
