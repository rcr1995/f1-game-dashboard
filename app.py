...
with tab_dash:
    ...
    with a1:
        ...
        last_tbl, _ = last_gp_table(latest_df, entity="Drivers")
        if last_tbl.empty:
    ...
    if not seasonleague_options:
        st.info(f"{tr(lang, 'not_enough')} {tr(lang, 'empty_hint')}")
        st.caption(f"{tr(lang, 'available_gp_rows')}: {len(d_gp)}")
        st.stop()
    ...
    if df_gp.empty:
        ...
        st.caption(f"{tr(lang, 'available_gp_rows')}: 0")
    else:
        ...
        st.markdown(f"### {tr(lang, 'exec_summary')}")
        ...
        title_fight_gap = float(st_table.iloc[0]["Points"] - st_table.iloc[1]["Points"]) if len(st_table) > 1 else np.nan
        form_l3 = form_table(df_stats, entity_col=entity_col, n_list=(3,))
        in_form = "-"
        if not form_l3.empty and "Pts L3" in form_l3.columns:
            ftop = form_l3.sort_values(["Pts L3", entity_col], ascending=[False, True]).head(1)
            in_form = str(ftop.iloc[0][entity_col])

        delta_tbl = position_delta_table(df_stats, entity_col=entity_col, n_momentum=3)
        biggest_mover = "-"
        if not delta_tbl.empty:
            dtop = delta_tbl.sort_values(["Delta", entity_col], ascending=[False, True]).head(1)
            biggest_mover = f"{dtop.iloc[0][entity_col]} ({int(dtop.iloc[0]['Delta']):+d})"
        ...
        c5.metric(tr(lang, "title_fight"), "-" if np.isnan(title_fight_gap) else f"{title_fight_gap:.0f}")
        c6.metric(tr(lang, "most_in_form"), in_form)
        st.caption(f"{tr(lang, 'biggest_mover')}: {biggest_mover}")
        ...
        with right:
            ...
            if wide.empty or long.empty:
                ...
                endpoint = last_pts.reset_index().rename(columns={"CumPoints": "CurrentPoints"})
                st.dataframe(endpoint, use_container_width=True, hide_index=True)

        st.markdown(f"### {tr(lang, 'tension')}")
        t1, t2 = st.columns([1.1, 1.4], gap="large")
        tension_metrics, tension_df, _ = championship_tension(df_stats, entity_col=entity_col)
        with t1:
        ...
        with t2:
            if not tension_df.empty and PLOTLY_OK:
                ...
                fig_gap.add_trace(go.Scatter(
                    x=tdf["EventLabel"], y=tdf["GapP1P2"], mode="lines+markers", name=tr(lang, "gap_p1_p2"), line={"color": THEME_CFG["positive"]}
                ))
                fig_gap.add_trace(go.Scatter(
                    x=tdf["EventLabel"], y=tdf["Top3Spread"], mode="lines+markers", name=tr(lang, "gap_top3"), line={"color": THEME_CFG["neutral"]}
                ))
        ...
        if delta_tbl.empty:
            ...
            st.dataframe(style_by_columns(delta_show, [tr(lang, "change"), tr(lang, "momentum_l3")]), use_container_width=True, hide_index=True)
            download_csv_button(delta_show, "gp_position_delta.csv", tr(lang, "download"))
